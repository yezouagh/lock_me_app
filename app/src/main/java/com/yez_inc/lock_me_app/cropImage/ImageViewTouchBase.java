...
abstract class ImageViewTouchBase extends ImageView {
    ...
    public void setImageRotateBitmapResetBase(final RotateBitmap bitmap, final boolean resetSupp) {
        ...
        if (resetSupp) {mSuppMatrix.reset();}
        setImageMatrix(getImageViewMatrix());
        mMaxZoom = maxZoom();
    }
    ...
    private void init() {setScaleType(ScaleType.MATRIX);}
    ...
    protected float getScale() {return getScale(mSuppMatrix);}
    ...
    protected void zoomIn() {zoomIn(SCALE_RATE);}
    protected void zoomOut() {zoomOut(SCALE_RATE);}
    protected void zoomIn(float rate) {
        if (getScale() >= mMaxZoom) {return;}     // Don't let the user zoom into the molecular level.
    ...
    protected void postTranslate(float dx, float dy) {mSuppMatrix.postTranslate(dx, dy);}
